...
function toastSafe(m){try{if(typeof toast==="function")toast(m);else console.log(m);}catch(e){}}
...
function guardarBackupAutomatico(motivo){
  ...
  var payload={meta:{motivo:motivo||"cambio",timestamp:new Date().toISOString(),url:location.href,version:"mejoras4-v2"},localStorage:recolectarLocalStorage()};
...
function fixCSSHidden(){
  ...
  if(!m4css)return;
  m4css.textContent=m4css.textContent.replace("#hoy-cal-m4g,#hoy-tasks-m4g{display:none!important}","/* hoy-cal y hoy-tasks: visibles */");
}
...
window.mejorarAgendaHoy=function(){
  ...
  for(var i=0;i<agPart.children.length;i++){
    ...
    if(!btn){
      var orig=row.querySelector("button:not(.hoy-cubrir-btn-m4)");if(orig)orig.remove();
      btn=document.createElement("button");btn.className="hoy-cubrir-btn-m4";btn.setAttribute("data-key",ev.key);
      btn.style.cssText="margin-left:auto;border-radius:6px;padding:3px 10px;cursor:pointer;font-size:.72rem;white-space:nowrap;flex-shrink:0;transition:all .2s;font-weight:600;border:1px solid #d1d5db;background:#f9fafb;color:#6b7280";
      row.appendChild(btn);
      btn.addEventListener("click",function(e){e.stopPropagation();var k=this.getAttribute("data-key");var cv=getCubiertos();if(cv[k])delete cv[k];else cv[k]=true;setCubiertos(cv);window.mejorarAgendaHoy();});
    }
...
function normalizarTextos(){
  ...
  try{
    ...
    while((node=walker.nextNode())){var v=node.nodeValue;Object.keys(repl).forEach(function(k){v=v.split(k).join(repl[k]);});node.nodeValue=v;}
    document.querySelectorAll("option").forEach(function(opt){if(opt.textContent.trim().toLowerCase()==="sofia")opt.textContent="Sofía";});
  }catch(e){}
}
...
function reforzarBackup(){
  try{
    ...
    if(!btn){
      btn=document.createElement("button");btn.type="button";btn.className="btn sm m4-backup-btn";btn.textContent="Backup";btn.onclick=function(){descargarBackup("manual");};
      var tr=document.querySelector(".tr")||document.querySelector(".topbar");if(tr)tr.insertBefore(btn,tr.firstChild);
    }else{btn.classList.add("m4-backup-btn");btn.title="Descargar copia de seguridad de los datos guardados en este navegador";}
  }catch(e){}
}
function sumarEstadoDatos(){
  var tr=document.querySelector(".tr");if(!tr||document.getElementById("m4-data-chip"))return;
  var chip=document.createElement("span");chip.id="m4-data-chip";chip.className="m4-data-chip";chip.textContent="Datos cuidados";chip.title="El panel guarda un backup automatico antes de acciones de borrado.";tr.insertBefore(chip,tr.firstChild);
}
function crearMobileBar(){
  ...
  [["hoy","Hoy"],["publicaciones","Agenda"],["calendario","Calendario"],["guardias","Guardia"],["equipo","Equipo"]].forEach(function(it){
    var b=document.createElement("button");b.type="button";b.setAttribute("data-m4nav",it[0]);b.textContent=it[1];b.onclick=function(){if(typeof window.nav==="function")window.nav(it[0],b);};bar.appendChild(b);
  });
...
function destacarHoy(){
  var hoy=document.getElementById("p-hoy");if(!hoy||document.getElementById("m4-home-band"))return;
  var band=document.createElement("div");band.id="m4-home-band";band.className="m4-home-band";
...
function envolverAccionesDestructivas(){
  function wrap(name,label){
    ...
    var wrapped=function(){if(!confirmarConBackup("¿Confirmás "+label+"?",label))return;return orig.apply(this,arguments);};
    wrapped._m4safe=true;window[name]=wrapped;
  }
...
function patchNav(){
  ...
  window.nav=function(id){
    ...
    setTimeout(function(){normalizarTextos();reforzarBackup();sumarEstadoDatos();envolverAccionesDestructivas();if(id==="hoy"){destacarHoy();window.mejorarAgendaHoy();}},250);
    return r;
  };
}
function observarCambios(){
  if(window._m4uxObs)return;
  try{var pending=false;window._m4uxObs=new MutationObserver(function(){if(pending)return;pending=true;setTimeout(function(){pending=false;normalizarTextos();reforzarBackup();envolverAccionesDestructivas();},350);});window._m4uxObs.observe(document.body,{childList:true,subtree:true});}catch(e){}
}

function inicializar(){
  fixCSSHidden();inyectarEstilosUX();normalizarTextos();reforzarBackup();sumarEstadoDatos();crearMobileBar();patchNav();observarCambios();envolverAccionesDestructivas();
  ...
  if(targetNode&&!window._m4obsM4){
    window._m4obsM4=new MutationObserver(function(){clearTimeout(window._m4obsTimerM4);window._m4obsTimerM4=setTimeout(function(){var phoy=document.getElementById("p-hoy");if(phoy&&phoy.style.display!=="none"&&phoy.offsetParent!==null){destacarHoy();window.mejorarAgendaHoy();}},200);});
    window._m4obsM4.observe(targetNode,{childList:true,subtree:true});
  }
  setTimeout(function(){destacarHoy();window.mejorarAgendaHoy();normalizarTextos();envolverAccionesDestructivas();},800);
}
