...
/* Tres calendarios oficiales: Intendente, PRENSA y Prensa Municipal. */
(function(){
  'use strict';
  var CALENDARS=[
    {
  ...
  function refreshControls(){
    if(!ready())return;
    var gc=byId('gcUrl');
    if(gc)gc.value=CALENDARS.map(function(c){return c.label}).join(' + ');
    var btn=byId('autoSyncBtn');
    if(btn)btn.textContent=state.config.googleAutoSync?' Auto-sync: ON (3 calendarios · cada 5 min)':' Auto-sync: OFF';
    var del=byId('gcDel');
    if(del)del.classList.toggle('hide',false);
  }
  function ensureConfig(){
    if(!ready())return false;
    state.config.googleCalendarId=CALENDARS[0].id;
    state.config.googleCalendarUrl=CALENDARS[0].url;
    state.config.googleCalendarIds=CALENDARS.map(function(c){return c.id});
    state.config.googleCalendarUrls=CALENDARS.map(function(c){return c.url});
    state.config.googleAutoSync=true;
    state.config.syncMode='auto';
    state.config.__calendarRealtimeConfigured=true;
    state.config.__threeCalendarsConfigured=true;
  ...
  async function syncOne(cal){
    var got=await fetchICS(cal);
    if(!got)return {label:cal.label,ok:false,imported:0,updated:0,removed:0,skipped:0};
    ...
    parseICS(got.text).forEach(function(ev){
      ...
      var id=eventId(cal,ev,dt);
      seen[id]=true;
      var data={
        id:id,date:dt.date,time:dt.time,desc:ev.SUMMARY||'(sin titulo)',loc:ev.LOCATION||'',
        type:cal.type,cover:false,gcalSource:true,gcalCalendarId:cal.id,
        gcalUID:ev.UID||'',notes:ev.DESCRIPTION||'',updated:Date.now()
      };
      var exists=state.events.find(function(x){
        return x.id===id||(x.gcalUID&&data.gcalUID&&x.gcalUID===data.gcalUID);
      });
      if(exists){
    ...
    state.events=state.events.filter(function(e){
      return !(e.gcalSource&&e.gcalCalendarId===cal.id&&!seen[e.id]);
    });
    return {
      label:cal.label,ok:true,imported:imported,updated:updated,
      removed:before-state.events.length,skipped:skipped,via:got.via
    };
  }
  async function syncAllCalendars(opts){
    opts=opts||{};
    ...
    try{
      ...
      var imported=results.reduce(function(n,r){return n+r.imported},0);
      var updated=results.reduce(function(n,r){return n+r.updated},0);
      var removed=results.reduce(function(n,r){return n+r.removed},0);
      if(failed.length){
  ...
  function schedule(){
    ...
    timer=setInterval(function(){syncAllCalendars({silent:true})},SYNC_MS);
  }
  function installOverrides(){
    ...
    window.autoSyncToggle=function(){
      ensureConfig();
      schedule();
      syncAllCalendars({silent:true});
      toastSafe('Auto-sync activado para 3 calendarios','suc');
    };
    schedule();
    setTimeout(function(){syncAllCalendars({silent:true})},1200);
    refreshControls();
    return true;
  }
  function boot(){
