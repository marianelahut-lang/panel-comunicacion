...
function recNuevoHTML(){var f=recForm,fo=recFobj(f.fid);var tOpts=recTipos.map(function(t){return '<option value="'+escH(t.nombre)+'"'+(f.tipo===t.nombre?' selected':'')+'>'+escH(t.nombre)+'</option>';}).join('');var fOpts=recFuncs.map(function(x){return '<option value="'+x.id+'"'+(f.fid==x.id?' selected':'')+'>'+escH(x.nombre)+'</option>';}).join('');var urgBtns=['Alta','Media','Baja'].map(function(u){var c=recUC(u),s=f.urg===u;return '<button onclick="recSetUrg('+JSON.stringify(u)+');" style="flex:1;padding:7px 4px;border:1.5px solid '+(s?c.c:'#e5e7eb')+';border-radius:8px;background:'+(s?c.bg:'transparent')+';color:'+(s?c.c:'#6b7280')+';cursor:pointer;font-size:12px;font-weight:'+(s?'500':'400')+'">'+u+'</button>';}).join('');var fPrev=fo?'<div style="display:flex;align-items:center;gap:10px;padding:8px 10px;background:#f9fafb;border-radius:8px;margin-bottom:12px;border:0.5px solid #e5e7eb"><div class="rec-av">'+escH(fo.nombre.charAt(0))+'</div><div><div style="font-size:13px;font-weight:500">'+escH(fo.nombre)+'</div><div style="font-size:11px;color:#16a34a">Cel: '+fo.tel+'</div></div></div>':'';var ok=!!(f.tipo&&f.fid&&f.desc);return '<div style="display:grid;grid-template-columns:1fr 1fr;gap:12px"><div class="rec-card"><p style="font-size:12px;font-weight:500;color:#6d28d9;margin:0 0 14px">DATOS DEL RECLAMO</p><div style="margin-bottom:10px"><label class="rec-lbl">Tipo *</label><select class="rec-inp" onchange="recSF('+JSON.stringify('tipo')+',this.value)"><option value="">Seleccionar...</option>'+tOpts+'</select></div><div style="margin-bottom:10px"><label class="rec-lbl">Direccion</label><input class="rec-inp" placeholder="Ej: Calle 9 de Julio 450" value="'+escH(f.dir)+'" oninput="recSF('+JSON.stringify('dir')+',this.value)"></div><div style="margin-bottom:10px"><label class="rec-lbl">Descripcion *</label><textarea class="rec-inp" style="height:72px;resize:vertical" oninput="recSF('+JSON.stringify('desc')+',this.value)">'+escH(f.desc)+'</textarea></div><div><label class="rec-lbl">Urgencia</label><div style="display:flex;gap:6px">'+urgBtns+'</div></div></div><div><div class="rec-card" style="margin-bottom:10px"><p style="font-size:12px;font-weight:500;color:#6d28d9;margin:0 0 12px">DATOS DEL VECINO</p><div style="margin-bottom:10px"><label class="rec-lbl">Nombre</label><input class="rec-inp" placeholder="Nombre completo" value="'+escH(f.vecino)+'" oninput="recSF('+JSON.stringify('vecino')+',this.value)"></div><div><label class="rec-lbl">Telefono</label><input class="rec-inp" placeholder="2983 123456" value="'+escH(f.telV)+'" oninput="recSF('+JSON.stringify('telV')+',this.value)"></div></div><div class="rec-card"><p style="font-size:12px;font-weight:500;color:#6d28d9;margin:0 0 12px">FUNCIONARIO *</p><div style="margin-bottom:10px"><select class="rec-inp" onchange="recSF('+JSON.stringify('fid')+',this.value)"><option value="">Seleccionar funcionario...</option>'+fOpts+'</select></div>'+fPrev+'<button onclick="recEnviar()" '+(ok?'':'disabled ')+'class="rec-wa" style="'+(ok?'':'background:#d1d5db;cursor:not-allowed')+'">Guardar y enviar por WhatsApp</button></div></div></div>';}
