...
nc2.querySelectorAll("span,div").forEach(function(e){e.remove();});
...
var m=oc&&oc.match(/['"]([\d]{4}-[\d]{2}-[\d]{2})['"]/);if(m&&!dateStr)dateStr=m[1];});
if(!dateStr){var hEl=gwd.querySelector(".gwdate,.gwhd");if(hEl)dateStr=hEl.getAttribute("data-date")||"";}
var cobDiv=null;
gwd.querySelectorAll("div").forEach(function(d){if(d.textContent.indexOf("Coberturas del")>-1)cobDiv=d;});
var cobs=[];
if(cobDiv){var rows=cobDiv.querySelectorAll(".gwact,.gwrow,li");
if(rows.length===0){Array.from(cobDiv.children).forEach(function(c,i){if(i>0){
var t2=c.children&&c.children[1]?c.children[1].children[0].textContent.trim():c.textContent.trim();
var tm=c.children&&c.children[1]&&c.children[1].children[1]?c.children[1].children[1].textContent.replace(/[^\d:]/g,"").trim():"00:00";
if(t2)cobs.push({title:t2,time:tm||"00:00"});}});
}else{rows.forEach(function(r){var t=r.textContent.trim();if(t)cobs.push({title:t,time:"00:00"});});}}
...
var dt=ev.start&&ev.start.dateTime?ev.start.dateTime:"";
var t="00:00";if(dt){var m=dt.match(/T(\d{2}:\d{2})/);if(m)t=m[1];}
...
var wf=_m4days[0]?_m4days[0].dateStr:"";var wl=_m4days[_m4days.length-1]?_m4days[_m4days.length-1].dateStr:"";
var wlabel="";
if(wf&&wl){var pa=wf.split("-");var pb=wl.split("-");
...
var allActs=getCalEvts(day.dateStr).concat(getPubs(day.dateStr)).concat(day.cobs.map(function(c){return {title:c.title,time:c.time,type:"cob"};}));
if(!isWE)allActs=allActs.filter(function(a){return a.time>="15:00";});
var cnt=allActs.length;
var cls="m4g-card"+(day.dateStr===today?" m4g-today":"")+(day.dateStr===_m4selDay?" m4g-active":"");
...
var pm=encodeURIComponent("Cobertura de prensa - "+fmtDate(day.dateStr)+"\n\nHola "+day.name+", tenes cobertura de prensa asignada para hoy.\nCualquier consulta coordinamos por aca.");
var pu=wp?"https://api.whatsapp.com/send"+String.fromCharCode(63)+"phone="+wp+"&text="+pm:"#";
...
grid+="<button class=\"m4g-prensa\" onclick=\"event.stopPropagation();window.open('"+pu+"','_blank')\">\uD83D\uDCF0 Cubrir con Prensa</button>";
grid+="</div>";
});
grid+="</div>";
...
var combined=getCalEvts(sd.dateStr).concat(getPubs(sd.dateStr)).concat(sd.cobs.map(function(c){return {title:c.title,time:c.time,type:"cob"};}));
...
var wlines=["\uD83D\uDEE1\uFE0F *Guardia del dia* \u2014 "+fmtDate(sd.dateStr),"","Hola "+sd.name+", sos la guardia *"+sd.role+"* hoy.","","\uD83D\uDCC5 Resumen del dia:"];
...
detHtml+="<div class=\"m4g-main\">";
detHtml+="<div class=\"m4g-detail\">";
...
detHtml+="<div class=\"m4g-sidebar\">";
detHtml+="<div class=\"m4g-resumen\"><h4>Resumen del dia</h4>";
...
var cd=hoy.querySelector("#hoy-cal-m4g");
if(!cd){cd=document.createElement("div");cd.id="hoy-cal-m4g";
cd.style.cssText="background:#fff;border-radius:12px;border:1px solid #e2d8f7;padding:12px;margin-top:10px;";
hoy.appendChild(cd);}
var evs=getCalEvts(todayISO());
if(!evs.length){cd.innerHTML="<div style=\"color:#aaa;font-size:.85rem;padding:8px 0\">Sin eventos de calendario hoy</div>";return;}
var h="<h4 style=\"margin:0 0 8px;font-size:.85rem;font-weight:700;color:#6c3fc5\">\uD83D\uDCC5 Eventos hoy del calendario</h4>";
evs.sort(function(a,b){return a.time<b.time?-1:1;});
evs.forEach(function(e){h+="<div style=\"display:flex;gap:8px;padding:5px 0;border-bottom:1px solid #f3f0fb\"><span style=\"color:#6c3fc5;font-weight:700;font-size:.8rem;min-width:42px\">"+e.time+"</span><span style=\"font-size:.83rem;color:#222\">"+e.title+"</span></div>";});
cd.innerHTML=h;}
function fixKanbanScroll(){
if(window._m4kanbanPatched)return;window._m4kanbanPatched=true;
var ork=window.renderKanban;
if(typeof ork==="function"){window.renderKanban=function(){var sp=window.scrollY;ork.apply(this,arguments);setTimeout(function(){window.scrollTo(0,sp);},50);};}}
...
document.querySelectorAll(".kanban-col,.kboard-col,[data-status]").forEach(function(c){
...
var on=window.nav;
if(typeof on==="function"){window.nav=function(id){
...
if(typeof os==="function"){window.syncGCal=function(){
window._m4gcal=[];
os.apply(this,arguments);
setTimeout(function(){
if(document.getElementById("p-guardias")&&document.body.classList.contains("m4tab-guardias")){renderM4G();}
inyectarHoyCalendario();
},2000);
;};}}
function init(){
interceptFetch();inyectarCSS();fixKanbanScroll();patchNav();patchGoGuardWeek();patchSyncGCal();
var tabs=["hoy","tablero","material","publicaciones","calendario","guardias","equipo","medios","reclamos","recursos"];
tabs.forEach(function(t){var el=document.getElementById("p-"+t);
if(el&&el.style.display!=="none"&&el.offsetParent!==null){setTabClass(t);}});
setTimeout(function(){renderM4G();inyectarHoyCalendario();hideRealizada();},600);
}
